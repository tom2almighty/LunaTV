...
export default function SearchSuggestions({
  ...
  return (
    <div
      ref={containerRef}
      className='bg-card border-border absolute left-0 right-0 top-full z-[600] mt-1 max-h-80 overflow-y-auto rounded-lg border shadow-lg'
    >
      {suggestions.map((suggestion) => (
        <button
          key={`related-${suggestion.text}`}
          onClick={() => onSelect(suggestion.text)}
          className='hover:bg-muted flex w-full items-center gap-3 px-4 py-3 text-left transition-colors duration-150'
        >
