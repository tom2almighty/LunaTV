...
export function PlayPlayerRuntime() {
  ...
  return (
    <PlayPageContainer>
      {/* 第一行：影片标题 */}
      <div className='app-panel rounded-[1.75rem] px-5 py-4 md:px-6'>
        <div className='flex items-center justify-between gap-3'>
          ...
          <button
            ...
            className='app-control text-foreground rounded-full px-3.5 py-2 text-sm font-medium transition-colors hover:text-[var(--accent)]'
          >
            返回搜索
          </button>
        </div>
      ...
      <div className='space-y-2'>
        ...
        <div className='hidden justify-end lg:flex'>
          <button
            ...
            className='app-control text-foreground group relative flex items-center space-x-1.5 rounded-full px-3.5 py-2 text-xs font-medium transition-all duration-200 hover:text-[var(--accent)]'
        ...
        >
          {/* 播放器 */}
          <div
            className={`app-panel h-full overflow-hidden rounded-[1.75rem] transition-all duration-300 ease-in-out ${
          ...
          >
            <div className='h-75 relative w-full lg:h-full'>
              <div
                ref={artRef}
                className='h-full w-full overflow-hidden rounded-[1.75rem] bg-black/70'
              ></div>

              {/* 换源加载蒙层 */}
              {isVideoLoading && (
                <div className='z-500 bg-[color:var(--overlay-heavy)]/95 absolute inset-0 flex items-center justify-center rounded-[1.75rem] backdrop-blur-xl transition-all duration-300'>
      ...
      {/* 详情展示 */}
      <div className='app-panel flex min-h-80 flex-col rounded-[1.75rem] p-6 md:p-7'>
        {/* 标题 */}
        ...
        {/* 关键信息行 */}
        <div className='text-foreground mb-5 flex shrink-0 flex-wrap items-center gap-2.5 text-sm'>
          {detail?.score && detail.score !== '0.0' && detail.score !== '0' && (
            <span className='border-warning/20 bg-warning/12 text-warning rounded-full border px-3 py-1 text-sm font-semibold'>
              {detail.score} 分
            </span>
          )}
          {detail?.class && (
            <span className='border-[var(--accent)]/20 bg-[var(--accent)]/12 rounded-full border px-3 py-1 font-medium text-[var(--accent)]'>
              {detail.class}
            </span>
          )}
          {(detail?.year || videoYear) && (
            <span className='app-control text-muted-foreground rounded-full px-3 py-1'>
              {detail?.year || videoYear}
            </span>
          )}
          {detail?.area && (
          ...
          {detail?.source_name && (
            <span className='app-control text-foreground rounded-full px-3 py-1'>
              {detail.source_name}
            </span>
          )}
          {detail?.type_name && (
          ...
          {videoDoubanId !== 0 && (
            <a
              ...
              className='border-success/25 bg-success/12 text-success rounded-full border px-3 py-1 font-medium transition-opacity hover:opacity-85'
            >
              豆瓣
            </a>
          )}
        ...
        {detail?.desc && (
          <div
            className='scrollbar-hide app-control text-foreground mt-1 min-h-0 flex-1 overflow-y-auto rounded-[1.5rem] px-4 py-4 pr-3 text-sm leading-7 md:text-base'
