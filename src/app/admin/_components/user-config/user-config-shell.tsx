...
import { useUserConfigActions } from './use-user-config-actions';
import { UserConfigContainer } from './user-config-container';
import {
...
// 用户配置组件
export interface UserConfigShellProps {
    ...
    await withLoading(`saveUserApis_${selectedUser.username}`, async () => {
      try {
        const res = await fetch(
          `/api/admin/users/${encodeURIComponent(selectedUser.username)}/api-access`,
          {
            method: 'PATCH',
  ...
  const handleUserAction = async (
    ...
    try {
      ...
      if (action === 'add') {
        ...
        endpoint = `/api/admin/users/${encodeURIComponent(targetUsername)}`;
        method = 'PATCH';
        body = { action };
      } else if (action === 'setAdmin' || action === 'cancelAdmin') {
        endpoint = `/api/admin/users/${encodeURIComponent(targetUsername)}/role`;
        method = 'PATCH';
        body = {
          role: action === 'setAdmin' ? 'admin' : 'user',
        };
      } else if (action === 'changePassword') {
        endpoint = `/api/admin/users/${encodeURIComponent(targetUsername)}/password`;
        method = 'PATCH';
        body = {
        ...
        <div>
          ...
          >
            <table className='divide-border min-w-full divide-y'>
              ...
              {(() => {
                const sortedUsers = [...config.UserConfig.Users].sort(
                  (a, b) => {
                    ...
                    {deletingUserGroup.affectedUsers.length > 0 ? (
                      <div className={warningPanelClass}>
                        ...
                        <div className='space-y-1'>
                          {deletingUserGroup.affectedUsers.map(
                            (user, index) => (
